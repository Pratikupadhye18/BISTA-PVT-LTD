...
function toggleMobileMenu() {
  ...
  if (!nav.classList.contains('mobile-open')) {
    document.querySelectorAll('.dropdown.mobile-dropdown-open').forEach(d => d.classList.remove('mobile-dropdown-open'));
  }
}

/* ===================== MOBILE NAV: DROPDOWN ACCORDION + AUTO-CLOSE ===================== */
(function () {
  // On mobile, tapping "Services ▾" expands/collapses the submenu instead of navigating away
  document.querySelectorAll('.dropdown > a').forEach(function (link) {
    link.addEventListener('click', function (e) {
      if (window.innerWidth <= 768) {
        e.preventDefault();
        const parent = this.closest('.dropdown');
        const wasOpen = parent.classList.contains('mobile-dropdown-open');
        document.querySelectorAll('.dropdown.mobile-dropdown-open').forEach(d => d.classList.remove('mobile-dropdown-open'));
        if (!wasOpen) parent.classList.add('mobile-dropdown-open');
      }
    });
  });

  // Tapping any actual page link closes the mobile menu
  document.querySelectorAll('.nav-links > li:not(.dropdown) > a, .dropdown-menu a').forEach(function (link) {
  ...
  // Resizing back to desktop clears any mobile-only state
...
(function () {
  const selector = [
    '.feature-card', '.division-card', '.step', '.service-photo-card', '.milestone',
    '.team-card', '.testimonial', '.pain-card', '.property-card', '.contact-info-card',
    '.contact-form', '.solve-item', '.cert-card', '.process-steps', '.division-body',
    '.map-box', '.solve-visual'
  ].join(', ');
